...
const Journal: React.FC<JournalProps> = ({ state }) => {
  ...
  // SYSTEM AUDIT: Filter history hanya untuk Broker yang dipilih
  const history = transactions.filter(t => t.status !== 'OPEN' && t.source === activeSource);
    ...
    <div className="space-y-8 animate-fadeIn pb-32 w-full max-w-full overflow-hidden">
       <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div>
            ...
            <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.4em] mt-2">Verified Institutional Audit Log: <span className="text-emerald-500">{activeSource.replace('_', ' ').toUpperCase()}</span></p>
          </div>
          <div className="flex gap-4">
             <div className="px-6 py-3 bg-slate-950/80 border border-white/5 rounded-2xl text-center shadow-inner">
                ...
                <p className="text-xl font-mono font-black text-indigo-400">{profitFactor === 'Infinity' ? '∞' : profitFactor}</p>
             </div>
             <div className="px-6 py-3 bg-slate-950/80 border border-white/5 rounded-2xl text-center shadow-inner">
       ...
       <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            ...
            { label: 'Net Cumulative Yield', val: `${stats.netPnL >= 0 ? '+' : ''}${stats.netPnL.toFixed(2)}`, sub: `${selectedCurrency.code} Base` }
          ].map((s, i) => (
       ...
       <div className="quantum-card rounded-[3.5rem] glass border-white/5 p-8 shadow-2xl overflow-hidden min-h-[500px] relative w-full">
          <div className="flex justify-between items-center mb-8 relative z-10">
             <h4 className="font-orbitron font-black text-xs text-white uppercase tracking-widest">Historical Transaction Matrix ({activeSource.replace('_', ' ').toUpperCase()})</h4>
             ...
             <table className="w-full text-left border-separate border-spacing-y-3">
                <thead>
                   <tr className="text-[8px] font-black text-slate-600 uppercase tracking-[0.2em]">
                      ...
                      <th className="px-6 py-2 text-right">Final PnL ({selectedCurrency.code})</th>
                ...
                <tbody className="font-mono text-[10px]">
                   {history.length === 0 ? (
                      <tr>
                         <td colSpan={6} className="text-center py-20 text-slate-700 uppercase font-black tracking-widest">Archive Empty. Execute trades on {activeSource.toUpperCase()} to generate logs.</td>
                      </tr>
                   ) : (
                      history.map(tx => (
                        <tr key={tx.id} className="group hover:bg-white/[0.02] transition-colors">
                           ...
                           <td className="px-6 py-4 bg-slate-950/40 border-y border-white/5">
                              <span className={`px-2 py-0.5 rounded-md text-[8px] font-black uppercase border ${tx.type.includes('BUY') ? 'text-emerald-500 border-emerald-500/20' : 'text-rose-500 border-rose-500/20'}`}>{tx.type.replace('AUTO_', '')}</span>
                           </td>
                           <td className={`px-6 py-4 bg-slate-950/40 border-y border-white/5 font-black text-right text-sm ${ (tx.pnl || 0) >= 0 ? 'text-emerald-400' : 'text-rose-500'}`}>
                              {tx.pnl ? `${tx.pnl >= 0 ? '+' : ''}${(tx.pnl * selectedCurrency.rate).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}` : '0.00'}
                           </td>
